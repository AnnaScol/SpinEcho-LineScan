...
% %% SELECT A LINE %% %
LINE_SELECTED = 25;
%26 gave line 26 and 27 in the kspace image?

%check what line based on centerline (eg, centerline for 48 points is 25)
...
gradAmp(1,timestepsREADOUTEvent) = G_r*(48/50); %X read out event 
adc(1,timestepsADCEvent) = 1:(xStepsImg); % adc event
% gradAmp(1,timestepsREADOUTEvent) = G_r*(48/50); %X read out event 



%%% ADDING SPOILING GRADIENTS
n=2;
...
gradAmp(3,round((pulsedurE/(dt))+1):((round(pulsedurR/(dt)))+((round(pulsedurR/(dt))/2)))) =  -G_s; %Z gradients in Tesla per meter

gradAmp(2,round((startR*(1/dt)))+(rfStepsR)) =  G_s; %Z gradients in Tesla per meter orig
% These dephase and rephase the 180 pulse 
gradAmp(2,round((startR*(1/dt))-((pulsedurR/(dt))/2)+(1:((pulsedurR/(dt))/2)))) =  3.2*G_s; %Z gradients in Tesla per meter
gradAmp(2,round((startR*(1/dt)))+(pulsedurR/(dt)+(1:((pulsedurR/(dt))/2)))) = 3.2*G_s; %Z gradients in Tesla per meter
...
subplot(6,1,2); plot(time,gradAmp(1,:),'r-','LineWidth',2);title('Full Sequence: Read Gradient'); ylim([-0.06 0.07]);
xlabel('time (s)'), ylabel('G_{r}(T/m)');grid on;
subplot(6,1,3); plot(time,gradAmp(2,:),'g-','LineWidth',2);title('Full Sequence: Phase Gradient');ylim([-0.06 0.091]);
...
subplot(6,1,5); plot(time,adc(1,:),'k-','LineWidth',2);title('ADC_{x} Events');ylim([0 50])
xlabel('time (s)'), ylabel('ADC Event Count');grid on;

%% Perform seuquence
kSpace  = zeros(yStepsImg,1); %vector since 1d line reading
durPE = 0.5e-3;
...
for i=1:zStepsSim
    ...
    for t=2:nTimeSteps 
        ...
        if(adc(1,t)>0)
            ...
            kSpace(adc(1,t),1) = kSpace(adc(1,t),1)...
...
figure;
Ispace = fftshift(ifft(fftshift(kSpace)));
subplot(2,1,1);imagesc(abs(Ispace));
title(sprintf('Magnitude Image for Center most Line (line %d)',LINE_SELECTED));
subplot(2,1,2);imagesc(log(abs((kSpace))),'CDataMapping','scaled'); 
title('Absolute kSpace');

save('LineImage1D.mat','Ispace')