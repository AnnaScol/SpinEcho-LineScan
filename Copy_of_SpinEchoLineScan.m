...
rfPulse(round((startR*10^6))+(rfStepsR)-1) = rfPulseR;

%Generate the readout + ADC
% timestepsADCEvent = 576:623;
% timestepsADCEvent=((TE/dt)-(24*10^-5)/dt):((TE/dt)-(25*10^-5)/dt);
timestepsREADOUTEvent = ((TE/dt)-(24*10^-5)/dt):((TE/dt)+(23*10^-5)/dt);

% adc(:,timestepsADCEvent) = linespace(1,) 1:(10^-5/dt):(48*10^-5/dt);
counter = ((TE/dt)-(24*10^-5)/dt)+1;
for v = 1:48
    adc(:,counter) = v;
    counter = counter+round((10^-5)/dt);
end
gradAmp(1,timestepsREADOUTEvent) = G_r; %X gradients in Tesla per meter


% Generate the phase encoding gradient
timestepsPHASEPrephaseEvent = ((TE/dt)-(74*10^-5)/dt):((TE/dt)-(25*10^-5)/dt);
...
timestepsREADOUTPrephaseEvent = ((TE/dt)-(74*10^-5)/dt):((TE/dt)-(25*10^-5)/dt);
...
gradAmp(3,round((startR*10^6))+(rfStepsR)-1) =  G_s; %Z gradients in Tesla per meter
gradAmp(3,round((startR*10^6)-((pulsedurR/(dt))/2)+(1:((pulsedurR/(dt))/2)))) =  -G_s; %Z gradients in Tesla per meter
gradAmp(3,round((startR*10^6))+(pulsedurR/(dt)+(1:((pulsedurR/(dt))/2)))) =  -G_s; %Z gradients in Tesla per meter


gradAmp(1,round(1:(pulsedurE/(dt)))) =  G_s; %Z gradients in Tesla per meter
gradAmp(1,round((pulsedurE/(dt))+1):((pulsedurR/(dt))+((pulsedurR/(dt))/2))) =  -G_s; %Z gradients in Tesla per meter
gradAmp(1,round((startR*10^6))+(rfStepsR)-1) =  G_s; %Z gradients in Tesla per meter
gradAmp(1,round((startR*10^6)-((pulsedurR/(dt))/2)+(1:((pulsedurR/(dt))/2)))) =  -G_s; %Z gradients in Tesla per meter
gradAmp(1,round((startR*10^6))+(pulsedurR/(dt)+(1:((pulsedurR/(dt))/2)))) =  -G_s; %Z gradients in Tesla per meter
